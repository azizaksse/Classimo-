...
const sortOptions = ["الكل", "الأحدث", "السعر الأقل", "السعر الأعلى"];
...
export default function ShopClient({ products }: ShopClientProps) {
  ...
  const categories = useMemo(() => {
    const unique = new Set<string>(products.map((p) => p.category || "غير مصنف"));
  ...
  return (
    <div className="space-y-12">
      <SectionHeader
        ...
        description="اطلع على أحدث المنتجات والتصاميم المتاحة لدينا مع تفاصيل دقيقة لكل منتج."
        lang="ar"
      />
      ...
      <div className="glass-panel rounded-[32px] p-6">
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            <p className="text-xs uppercase tracking-[0.3em]">الفئة</p>
            <select
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.3em]">بحث</p>
            <Input placeholder="مثال: من 20,000 إلى 60,000 دج" />
          </div>
        </div>
        <div className="mt-4 rounded-2xl border border-white/10 p-4 text-sm">
          استخدم عوامل التصفية للحصول على المنتجات التي تناسب ذوقك أو ميزانيتك.
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        {filteredProducts.length === 0 ? (
          <div className="col-span-full rounded-2xl border border-white/10 p-6 text-center text-sm text-white/70">
            لم يتم العثور على منتجات مطابقة للفلتر.
          </div>
        ) : null}
      </div>
        ...
        {cartItemsDetail.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <Button className="w-full">إتمام الشراء (COD)</Button>
